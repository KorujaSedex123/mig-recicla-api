...
public class EmbalagemRetornoController {
    ...
    public EmbalagemRetorno adicionar(@RequestBody EmbalagemRetorno embalagem) {
        // Se o status não for enviado, definimos como PENDENTE por padrão
    ...
    public ResponseEntity<EmbalagemRetorno> atualizarStatus(@PathVariable Long id, @RequestBody StatusDevolucao novoStatus) {
        return repository.findById(id)
                .map(embalagem -> {
                    embalagem.setStatus(novoStatus);
                    return ResponseEntity.ok(repository.save(embalagem));
                })
